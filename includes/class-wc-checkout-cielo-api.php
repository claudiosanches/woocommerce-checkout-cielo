...
class WC_Checkout_Cielo_API {
	...
	protected function get_shipping_data( $order ) {
		...
		if ( 0 < $order->get_total_shipping() ) {
			...
			//$data['shipping_type'] = '3'; <- Coloca frete grátis. Estou passando para retirada em loja.

			$data['shipping_type'] = '4';
	...
	public function get_options_data( $order ) {

		$data     = array();

		$data['ReturnUrl'] = esc_url( wc_get_endpoint_url( 'order-received', '', get_permalink( wc_get_page_id( 'checkout' ) ) ) ).'?order_idd='.$order->id;
	...
	public function get_transaction_data( $order ) {
		...
		$options_data = $this-> get_options_data( $order );

		$data          = apply_filters( 'wc_checkout_cielo_transaction_data', array_merge( $data, $customer_data, $shipping_data, $products_data, $discount_data, $options_data ) );
